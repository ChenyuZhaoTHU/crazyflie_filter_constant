...
file_label_gui = "/home/nuc0428/project/AirTouch/crazyflie_filter_constant/control/sig.csv"


state_flag = 0
def edge_detection(imu_data, state_flag):
    height_list = imu_data[:, -1]
    
    print(height_list[-1])
    print("\n")
    if(height_list[-1]-height_list[-5]<-30 and state_flag == 1):
        with open(file_label_gui, 'w') as f:
            f.write('2')
            state_flag = 2
    # elif(height_list[-1]-height_list[-5]>30 and state_flag ==2):
    #     with open(file_label_gui, 'w+') as f:
    #         f.write('3')
    #         state_flag = 3
    elif(height_list[-1]-height_list[-5]<-30 and state_flag == 2):
...
def process_imu_data(timestamp, data, logconf):
    ...
    # detection th edge and write the label to the file
    edge_detection(imu_data, state_flag)
...
if __name__ == '__main__':
    ...
    with SyncCrazyflie(uri, cf=Crazyflie(rw_cache='./cache')) as scf:
        cf = scf.cf
        with PositionHlCommander(scf, x=0, y=0, default_height=0.7, controller=PositionHlCommander.CONTROLLER_PID) as mc:
